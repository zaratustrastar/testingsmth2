...
function splitModule(mode) {
  const steps = mode === 'lend'
    ? ['Pay USDC', 'Receive P', 'Receive repayment or redeem collateral']
    : ['Lock collateral', 'Mint P + N', 'Sell P for USDC', 'Keep N to repay and reclaim']
  return `<div class="split-module"><div class="split-diagram"><div class="split-top">Collateral</div><div class="split-line"></div><div class="split-legs"><div><strong>P</strong><span>Lender claim</span></div><div><strong>N</strong><span>Reclaim option</span></div></div></div><div class="split-copy"><ol>${steps.map((step) => `<li>${step}</li>`).join('')}</ol></div></div>`
}
