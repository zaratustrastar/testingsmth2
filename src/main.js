import { BrowserProvider, Contract, JsonRpcProvider, formatUnits, getAddress, isAddress, parseEther, parseUnits } from 'https://cdn.jsdelivr.net/npm/ethers@6.13.5/+esm'
...
function fallbackProvider() { return new JsonRpcProvider(BASE_RPC) }
...
async function readContract(address, abi) { return new Contract(address, abi, fallbackProvider()) }
...
function renderBorrow() {
  $('view').innerHTML = `<section class="borrow-layout">
    <div class="card form-card">
      ...
      <label>Amount to lock <strong id="amountLabel" class="term-label">0%</strong></label><div class="amount-row"><input id="lockAmount" placeholder="0.00"></div><input id="amountPercent" type="range" min="0" max="100" value="0">
      ...
      <label>Term ${infoTip()} <strong id="termLabel" class="term-label">180 days</strong></label>
      <input id="termDays" type="range" min="1" max="365" value="180">
    ...
    <aside><div class="card preview-card"><h2>Position preview</h2><div id="borrowPreview"></div><div class="important">${infoTip()}<div><strong>Important</strong><p>If not repaid by the deadline, P holders can redeem the collateral fallback.</p></div></div></div><div class="card split-card"><h2>How it works</h2>${splitModule('borrow')}</div></aside>
  </section>`
  let token = { symbol: 'TOKEN', name: '', decimals: 18, balance: 0n, loaded: false }
  let tokenAddress = ''
  const daysToDate = (days) => { const d = new Date(); d.setDate(d.getDate() + Number(days)); return d.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) }
  const update = () => {
    const lock = Number($('lockAmount').value || 0), raise = Number($('raiseUsdc').value || 0), repay = Number($('repayUsdc').value || 0), days = Number($('termDays').value || 180)
    $('termLabel').textContent = `${days} ${days === 1 ? 'day' : 'days'}`
    $('amountLabel').textContent = `${$('amountPercent').value || 0}%`
    const deadline = daysToDate(days)
    $('borrowPreview').innerHTML = previewRows([
      ['You lock', `${money(lock)} ${token.symbol}`], ['You receive', `${money(raise)} USDC when funded`], ['You keep', 'reclaim collateral option'], ['Repay to reclaim', `${money(repay)} USDC`], ['Deadline', deadline],
    ])
  }
  $('fetchToken').onclick = async () => {
    ...
    const erc20 = await readContract(getAddress(input), ERC20_READ_ABI)
    ...
    $('tokenBox').innerHTML = `<div><strong>${symbol}</strong><small>${name}</small></div><div class="balance"><small>Wallet balance</small><strong>${fmt(balance, Number(decimals), 8)}</strong></div>`
    $('amountPercent').value = '0'
    update()
  }
  $('amountPercent').oninput = () => { $('lockAmount').value = fmt((token.balance * BigInt($('amountPercent').value || 0)) / 100n, token.decimals, 8); update() }
  $('lockAmount').oninput = () => { update() }
  ;['raiseUsdc', 'repayUsdc', 'termDays'].forEach((id) => $(id).oninput = update)
  $('createBorrow').onclick = async () => {
    ...
    const days = Number($('termDays').value || 180)
    const maturity = BigInt(nowSec() + days * 86400)
...
function splitModule(mode) {
  if (mode === 'lend') {
    return `<div class="lend-flow">
      <div class="lend-step"><strong>USDC</strong><span>Fund with USDC</span><small>Provide USDC to fund the position.</small></div>
      <div class="flow-arrow">→</div>
      <div class="lend-step"><strong>P</strong><span>Receive P claim</span><small>P represents your right to the agreed repayment.</small></div>
      <div class="flow-arrow">→</div>
      <div class="outcome-stack"><div><strong>Borrower repays</strong><small>Receive the agreed USDC repayment.</small></div><div><strong>Borrower does not repay</strong><small>Redeem the locked collateral.</small></div></div>
    </div>`
  }
  const steps = [
    ['Lock your collateral', 'Deposit your token to open the position.'],
    ['Create P and N', 'Your collateral is split into two linked tokens.'],
    ['Sell P for USDC', 'Selling P gives you the USDC funding.'],
    ['Keep N', 'N gives you the right to repay later and reclaim your collateral.'],
  ]
  return `<div class="split-module borrower-how"><div class="split-diagram"><div class="split-top">Collateral</div><div class="split-line"></div><div class="split-legs"><div><strong>P</strong><span>Sold for USDC</span><small>Lender claim</small></div><div><strong>N</strong><span>Keep to reclaim</span><small>Borrower right</small></div></div></div><div class="split-copy"><ol>${steps.map(([title, copy]) => `<li><strong>${title}</strong><span>${copy}</span></li>`).join('')}</ol><div class="outcomes"><div><strong>If you repay by the deadline</strong><span>you get your collateral back</span></div><div><strong>If you do not repay</strong><span>the lender can redeem the collateral</span></div></div></div></div>`
}
